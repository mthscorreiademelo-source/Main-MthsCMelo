...
- 📅 **Agenda** — calendário de eventos estilo Google Calendar: **grade de tempo multi-dia** com seletor **Dia / 3 dias / 4 dias / Semana** (3 padrão), colunas por dia, régua de horas, **linha do agora** e faixa de dia inteiro. **Eventos** como blocos coloridos: crie **tocando ou arrastando** um intervalo na grade, **mova** e **redimensione** os blocos; editor com título, dia inteiro, início/fim, cor, local e descrição. **Time-blocking de tarefas**: tarefas com data e hora aparecem como blocos na própria grade (com ✓, abrem o editor de tarefa) e ao **arrastar/redimensionar** gravam `horario`/`duração` de volta na tarefa; tarefas com data sem hora ficam na faixa de dia inteiro. Card **"Agenda de hoje"** e tile de próximo evento no dashboard. Tudo local-first, sincroniza e entra no backup
