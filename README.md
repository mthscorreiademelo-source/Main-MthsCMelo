...
- 📚 **Biblioteca** — rastreador de leituras + agregador de arquivos: estantes **Quero ler · Lendo · Lido · Abandonei**, para livros, quadrinhos e mangás; adicione **importando o arquivo** (EPUB/PDF/CBZ — extrai **título, autor e capa** automaticamente) ou só o registro; por obra: capa, **progresso (%)**, **nota (estrelas)**, **resenha**, gêneros; busca e filtros na estante; card **"Leitura"** no Hoje com o que você está lendo. Os **metadados e a capa sincronizam** entre aparelhos; o **arquivo em si fica local** (leitura offline). *Leitor embutido (EPUB/PDF/CBZ) na próxima fase.*
